...
// States
import { DEFAULT_CHAIN, formatAddress, getEtherscanAddressURL, MetaMaskConnector, useWalletContext } from "../Wallet";
...
const ButtonConnectWalletDesktop: FunctionComponent<ButtonConnectWalletDesktopProps> = ({}) => {
    ...
    // Connect wallet
    const connect = async function (c: InjectedConnector) {
    ...
    return (
        <>
            ...
            {showConnectWallet && (
                <Button className="h-[40px] px-2 py-2" onClick={() => setIsOpen(true)}>
                    Connect Wallet
                </Button>
            )}
            ...
            {showAccountData && account && (
                <Popover>
                    <Popover.Button as={Button} ref={setReferenceElement1} className="h-[40px] px-2 py-2">
                        ...
                        <EnsLabel address={account} renderAddress={(address) => formatAddress(address)} />
                    </Popover.Button>
                    <Popover.Panel ref={setPopperElement1} style={popper1.styles.popper} {...popper1.attributes.popper} className="z-100 flex w-[241px] flex-col border border-[#E93BF8] bg-gray-dark-2">
                        {({ close }) => {
                            return (
                                <>
                                    ...
                                    <div className="mt-2 flex flex-col space-y-4 pb-4">
                                        <div className="flex flex-row justify-between px-4 text-sm leading-4">
                                            <a href={getEtherscanAddressURL(chain.chain, account)} className="text-gray-light-12 hover:underline dark:text-gray-dark-12" target="_blank" rel="noopener noreferrer">
