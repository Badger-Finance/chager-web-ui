...
export function useVaultApi() {
    ...
    const getVaultDepositorData = useCallback(async (vaultAddress: string, account: string): Promise<VaultDepositorInfo | null> => {
        ...
        const depositorData = await chadgerRegistryContract.getVaultDepositorData(
            vaultAddress,
            account,
        );
    ...
    const getAllVaultsDepositorData = useCallback(async (account: string): Promise<VaultDepositorInfo[]> => {
        ...
        const depositorData = await multicallProvider.allDict(
            vaultAddresses.map((vaultAdress) => {
                return {
                    vault: vaultAdress,
                    data: chadgerRegistryMulticallContract.getVaultDepositorData(
                        vaultAdress,
                        account,
                    ),
                };
    ...
    const getVaultBaseData = useCallback(async (vaultAddress: string): Promise<VaultBaseInfo | null> => {
        const multicallProvider = new MulticallProvider(provider, chain.chain.id);
        ...
        try {
            ...
            const rewardTokenAddresses = _.chain(vaultData.apr.map((a: any) => a.token) as string[])
                ...
                .uniq()
                .value()
            const tokenInfo = await multicallProvider.allDict(rewardTokenAddresses.map((address) => {
    ...
    const getAllVaultsBaseData = useCallback(async (): Promise<VaultBaseInfo[]> => {
        ...
        try {
            ...
            const vaultInfo = await P.map(vaultStrategyInfo, (info) => {
                return (async () => {
                    try {
